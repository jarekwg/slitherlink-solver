...
class Grid:
    ...
    def getLineVal(self, r, c, vertical):
        if 0 <= r < self.ROWS+1*(not vertical) and 0 <= c < self.COLS+1*vertical:
            if vertical:
    ...
    def countLinesSurroundingCell(self, r, c, key):
        ...
        count = 0
        if self.getLineVal(r,c,True) == key: count += 1
        if self.getLineVal(r,c+1,True) == key: count += 1
        if self.getLineVal(r,c,False) == key: count += 1
        if self.getLineVal(r+1,c,False) == key: count += 1
        return count
    ...
    def countLinesSurroundingDot(self, r, c, key):
        ...
        count = 0
        if self.getLineVal(r,c,True) == key: count += 1
        if self.getLineVal(r-1,c,True) == key: count += 1
        if self.getLineVal(r,c,False) == key: count += 1
        if self.getLineVal(r,c-1,False) == key: count += 1
        return count
    ...
    def sendLineOutThroughCorner(self, r, c, corner):
        ...
        # Move into next cell.
        r += self.CELL_POS_OVER_CORNER[corner]['row']
        c += self.CELL_POS_OVER_CORNER[corner]['col']
    ...
    def indecisiveTwo(self, r, c, corner, assumeUniqueSoln):
        ...
        # If assuming a unique solution exists, check if there are any numbers adjacent to this two.
    ...
    def findOtherEnd(self, r, c, cameFrom):
        for d in range(4):
            ...
            if self.getLineVal(rx,cx,self.LINE_POS_FROM_DOT_IN_DIRECTION[d]['vertical']) == Grid.LINE_KEYS['LINE'] and d != cameFrom:
                    ...
                    if val == '1':
                        for corner in range(4):
                            ...
                            if self.countCornerLinesOuter(r, c, corner, Grid.LINE_KEYS['NOT_ALLOWED']) == 2:
                                ...
                                self.sendLineOutThroughCorner(r, c, Grid.getOppositeCorner(corner))
                            # Line coming in.
                            if self.countCornerLinesOuter(r, c, corner, Grid.LINE_KEYS['NOT_ALLOWED']) == 1 and self.countCornerLinesOuter(r, c, corner, Grid.LINE_KEYS['LINE']) == 1:
                                self.setCornerLinesInner(r, c, Grid.getOppositeCorner(corner), Grid.LINE_KEYS['NOT_ALLOWED'])
                    # Tricks for Twos:
                    if val == '2':
                        for corner in range(4):
                            # Dead end corner.
                            if self.countCornerLinesOuter(r, c, corner, Grid.LINE_KEYS['NOT_ALLOWED']) == 2 and self.countLinesSurroundingCell(r, c, Grid.LINE_KEYS['NO_LINE']) == 4:
                                self.indecisiveTwo(r, c, Grid.getOppositeCorner(corner), assumeUniqueSoln)
                            # Line coming in.
                                ...
                                if self.countCornerLinesOuter(r, c, corner, Grid.LINE_KEYS['NOT_ALLOWED']) == 1:
                                    ...
                                    while True:
                                        if self.countCornerLinesOuter(rx, cx, Grid.getOppositeCorner(corner), Grid.LINE_KEYS['LINE']) == 1:
                                            ...
                                            break
                                        # Move into next cell.
                                        rx += self.CELL_POS_OVER_CORNER[corner]['row']
                                        cx += self.CELL_POS_OVER_CORNER[corner]['col']
